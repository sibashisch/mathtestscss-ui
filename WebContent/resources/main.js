...
const PARAM_EXAM_NAME = 'mathtest-examname';
...
let _saveUserData = function (key, val) {
	try {
...
let _clearUserData = function (key) {
	try {
...
let _showLoader = function(modalToHide) {
	var loaderElemHtml = 
				...
				+ '<div class="modal-content">'
					+ '<div class="card col-sm-12">'
						+ '<div class="card-body" style="text-align: center">'
							+ '<h5>Test Centre</h5>'
					    	+ '<img src="resources/images/loader.gif" alt="Please Wait ..." /> <br />'
					    	+ '<h5>Please Wait</h5>'
						+ '</div>'
					+ '</div>'
				+ '</div>'
	...
	if ($('#loader-modal').length <= 0) {
		$('body').append($(loaderElemHtml));
	}
	$('#loader-modal').modal();
}

let _hideLoader = function (modalToHShow) {
	try {
...
let _ajaxCall = function (sourceModalId, url, method, data, dataType, errorDiv, successCallBack) {

	_showLoader(sourceModalId);

	let customHeaders = {};
	if (_getUserData(USER_AUTH_KEY) !== null) {
		customHeaders  = {Authtoken: _getUserData(USER_AUTH_KEY)};
	}
	...
	$.ajax({
		...
		success: function (response) {
			if (response && response.status && response.status === 'success') {
				...
				$('#'+errorDiv).html (response.exception);
				_logError (response);
			} else {
				$('#'+errorDiv).html (ERROR_MESSGAE);
				_logError (response);
			}
			_hideLoader (sourceModalId);
		},
...
let _logout = function (sourceModalId, errorDiv) {
	_ajaxCall (sourceModalId, '/user/logout', 'get', {}, 'json', errorDiv, function(resp) {
		_clearUserData(USER_AUTH_KEY);
		...
		_clearUserData(PARAM_EXAM_NAME);
...
let _createTimePicker = function (elemId, opt) {
	let limit;
	if (opt === 'hour') 
		limit = 24;
	else if (opt === 'min')
